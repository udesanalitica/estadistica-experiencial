---
title: "Análisis de asociación entre variables"
editor: 
  markdown: 
    wrap: 72
---

En los capítulos anteriores se estudiaron las características de las
variables de manera individual, considerando aspectos como la tendencia
central, la dispersión, la posición y la forma de las distribuciones.
Sin embargo, en los problemas reales las variables rara vez se comportan
de manera aislada. Con frecuencia, el interés estadístico consiste en
comprender **cómo dos o más características se relacionan, varían
conjuntamente o permiten explicar el comportamiento de un fenómeno**.

Por ejemplo, en un estudio antropométrico puede resultar de interés
establecer si la **altura corporal se relaciona con el peso**, si
determinadas medidas corporales presentan patrones conjuntos, si existen
diferencias entre grupos o si algunas características pueden contribuir
a explicar el comportamiento de otra variable. Este tipo de preguntas
requiere pasar de la descripción individual de los datos al estudio de
las **relaciones y asociaciones entre variables**.

El **análisis de asociación** reúne herramientas estadísticas orientadas
a explorar, describir, cuantificar e interpretar estas relaciones.
Cuando el estudio se concentra específicamente en dos variables se habla
de **análisis bivariado**. Dependiendo de la naturaleza de las variables
y del propósito del análisis, pueden estudiarse relaciones entre
variables cuantitativas, asociaciones entre variables categóricas,
diferencias entre grupos o relaciones de tipo explicativo y predictivo.

Es importante distinguir que **asociación no significa causalidad**.
Identificar que dos variables presentan un patrón conjunto no implica
que una sea necesariamente la causa de la otra. La interpretación debe
realizarse considerando el contexto del estudio, el diseño mediante el
cual se obtuvieron los datos y las posibles variables adicionales
involucradas.

En este capítulo se desarrollarán herramientas como los **diagramas de
dispersión, la covarianza, la correlación de Pearson, la correlación de
Spearman, las tablas de contingencia y los modelos de regresión
lineal**. Estas técnicas permiten estudiar la dirección, intensidad y
estructura de las relaciones observadas, así como construir modelos
básicos para describir o explicar el comportamiento conjunto de las
variables.

El capítulo utilizará una **base de datos antropométrica de estudiantes
universitarios**, que contiene variables cuantitativas y categóricas
relacionadas con características corporales. A partir de estos datos se
desarrollarán ejemplos reproducibles en R que permitirán conectar
progresivamente la representación gráfica, las medidas de asociación y
los modelos estadísticos.

El enfoque será fundamentalmente experiencial: cada concepto será
acompañado por su interpretación, representación gráfica y aplicación
computacional. De esta manera, el estudiante podrá avanzar desde una
pregunta sobre la relación entre variables hasta la selección de la
técnica estadística apropiada, su implementación en R y la
interpretación de los resultados.

Este análisis constituye además una transición hacia métodos
estadísticos de mayor complejidad. La comprensión de las relaciones
entre variables proporciona fundamentos para estudiar posteriormente
modelos de regresión múltiple, técnicas multivariadas y procedimientos
utilizados en analítica de datos y modelamiento estadístico.

## Tipos de análisis de asociación según la naturaleza de las variables

La técnica estadística apropiada para estudiar una asociación depende
tanto de la **naturaleza de las variables** como del **objetivo del
análisis**. Antes de calcular cualquier medida o ajustar un modelo, es
necesario identificar si las variables son cuantitativas o categóricas y
determinar qué tipo de relación se desea explorar.
...
La siguiente tabla resume las principales situaciones que pueden
presentarse en el análisis de asociación:

| Variables involucradas | Pregunta principal | Herramientas iniciales |
|----|----|----|
| Cuantitativa -- Cuantitativa | ¿Existe relación?, ¿en qué dirección y con qué intensidad? | Dispersión, covarianza, Pearson, Spearman, regresión |
| Categórica -- Categórica | ¿Las categorías presentan asociación? | Tablas de contingencia y medidas o pruebas de asociación |
| Cuantitativa -- Categórica | ¿La distribución de la variable cuantitativa cambia entre grupos? | Boxplot, estadísticos por grupo y comparación |
| Una respuesta y varias explicativas | ¿Varias variables contribuyen conjuntamente a explicar una respuesta? | Regresión lineal múltiple |
...
En este capítulo se abordarán ejemplos correspondientes a varias de
estas situaciones, integrando representaciones gráficas, medidas de
asociación y modelos estadísticos básicos mediante implementaciones
desarrolladas en R.

## Ruta conceptual del capítulo

El recorrido propuesto sigue una secuencia progresiva:

**explorar visualmente → cuantificar la variación conjunta →
estandarizar la asociación → modelar la relación → estudiar asociaciones
entre categorías**.

Esta secuencia permite comprender no solo cómo calcular cada
estadístico, sino también qué pregunta responde, bajo qué condiciones
puede utilizarse y cómo interpretar sus resultados dentro del contexto
del problema.

## Conjunto de datos de trabajo

A lo largo de este capítulo se utilizará un mismo conjunto de datos con
el propósito de ilustrar las diferentes técnicas de análisis de
asociación de manera integrada y progresiva. El uso de una misma base
facilita la comparación de resultados y permite comprender cómo
distintas herramientas estadísticas pueden estudiar una realidad desde
perspectivas complementarias.

La base contiene información antropométrica correspondiente a
estudiantes universitarios e incluye variables cuantitativas y
categóricas. Entre las variables utilizadas en los ejemplos del capítulo
se encuentran la **altura**, el **peso corporal**, el **índice de masa
corporal (IMC)**, la **longitud de la pierna**, la **longitud del pie**,
la **longitud de la mano** y el **género**, además de variables
derivadas de clasificación antropométrica presentes en el conjunto de
datos.

Estas variables permiten desarrollar ejemplos de relaciones entre
variables cuantitativas, asociaciones entre variables categóricas,
comparaciones entre grupos y modelos de regresión.
...
respuesta ($Y$) y **una única variable explicativa** ($X$). Su forma
general es:
...
donde $\beta_0$ representa el intercepto, $\beta_1$ cuantifica el cambio
esperado en $Y$ por cada unidad de cambio en $X$, y $\varepsilon$
representa el componente aleatorio no explicado por el modelo. En este
capítulo, la relación entre altura y peso corporal se utiliza como
ejemplo de regresión lineal simple porque interviene una sola variable
predictora: la altura.

Esta denominación permite diferenciarla de la **regresión lineal
múltiple**, que incorpora dos o más variables explicativas en un mismo
modelo.
...
La diferencia central entre ambos modelos se encuentra en el número de
variables explicativas. La **regresión lineal simple** utiliza una sola
variable predictora, mientras que la **regresión lineal múltiple**
incorpora dos o más predictores para explicar conjuntamente el
comportamiento de la variable respuesta.

| Modelo | Número de variables explicativas | Forma general |
|----|---:|----|
| Regresión lineal simple | 1 | $Y = \beta_0 + \beta_1X + \varepsilon$ |
| Regresión lineal múltiple | 2 o más | $Y = \beta_0 + \beta_1X_1 + \cdots + \beta_kX_k + \varepsilon$ |
...
En el análisis de asociación, la selección de la técnica estadística
depende principalmente del tipo de variables que se desean analizar, del
objetivo del estudio y de los supuestos que cumplen los datos
(@fig-dosvariables). Por esta razón, antes de aplicar cualquier
procedimiento es necesario identificar si las variables son
cuantitativas o categóricas, revisar su distribución, explorar la forma
de la relación y verificar la presencia de valores atípicos.

![Esquema para la selección de técnicas de análisis de asociación según
la naturaleza de las
variables.](img/dosvariables.png){#fig-dosvariables}
...
La relación entre ambos conceptos puede expresarse de manera directa: la
**correlación de Pearson es una covarianza estandarizada**. En su forma
muestral:
...
El **numerador** corresponde a la covarianza entre $X$ y $Y$, que
refleja el sentido en que ambas variables varían conjuntamente. El
**denominador** corresponde al producto de sus desviaciones estándar y
actúa como un factor de estandarización. Gracias a esta estandarización,
el coeficiente de correlación no depende de las unidades originales de
medida y queda acotado entre $-1$ y $1$.

```{r}
#| label: fig-cov-correlacion
#| fig-cap: "Relación conceptual entre la covarianza y la correlación de Pearson."
#| echo: false
#| message: false
#| warning: false
...
text(
  5, 9,
  "Correlación de Pearson = covarianza estandarizada",
  font = 2, cex = 1.2
)

text(2.1, 5.9, "r(X,Y) =", cex = 1.5)
text(5.0, 6.7, "Cov(X,Y)", cex = 1.4, font = 2)
segments(3.7, 6.05, 6.3, 6.05, lwd = 2)
text(5.0, 5.25, "sX × sY", cex = 1.35)

arrows(5.0, 8.0, 5.0, 7.15, length = 0.08, lwd = 1.5)
text(5.0, 8.45, "Numerador: covarianza", font = 2, cex = 1.0)

arrows(8.0, 4.1, 6.2, 5.15, length = 0.08, lwd = 1.5)
text(8.0, 3.45, "Denominador:
estandariza por la escala", cex = 0.95)

text(
  5, 1.5,
  "La covarianza conserva las unidades; la correlación no tiene unidades y está entre -1 y 1.",
  cex = 0.9
)
box()
```
...
La covarianza y la correlación describen la variación conjunta de dos
variables, pero **no son equivalentes**. La covarianza conserva las
unidades de las variables y no posee límites fijos; por ello, su
magnitud no debe interpretarse como si estuviera en una escala de $-1$ a
$1$. La correlación divide la covarianza por el producto de las
desviaciones estándar y produce una medida adimensional comparable entre
diferentes pares de variables.
:::

![Escala conceptual para interpretar la dirección y magnitud de un
coeficiente de correlación.](img/correlacion.png){#fig-correlacion}
...
![Esquema conceptual de la correlación de Spearman basada en
rangos.](img/corspearman.png){#fig-corspearman}
...
## Otras herramientas para explorar la asociación
...
Esta herramienta constituye generalmente el primer paso del análisis de
asociación entre variables cuantitativas, ya que permite identificar
visualmente patrones, tendencias, agrupamientos y posibles valores
atípicos antes de aplicar medidas numéricas de correlación o modelos de
regresión (@fig-dispersion). A partir de un diagrama de dispersión
pueden identificarse diferentes comportamientos entre las variables:

![Ejemplos de patrones de asociación que pueden observarse en diagramas
de dispersión.](img/dispersion.png){#fig-dispersion}
...
correlación lineal y de la regresión. Para una muestra, puede expresarse
como:
...
Esta expresión muestra que cada observación aporta a la covarianza a
través del producto $(x_i-\bar{x})(y_i-\bar{y})$. La posición de cada
punto respecto a las medias de $X$ y $Y$ permite interpretar el signo de
esa contribución:
...
Por tanto, cuando predominan puntos en los cuadrantes I y III, la
covarianza tiende a ser positiva; cuando predominan en los cuadrantes II
y IV, tiende a ser negativa. Si las contribuciones positivas y negativas
se compensan, la covarianza puede aproximarse a cero.

```{r}
#| label: fig-covarianza-cuadrantes
#| fig-cap: "Interpretación geométrica de la covarianza. Las líneas representan las medias de X y Y; los cuadrantes I y III generan contribuciones positivas, mientras que II y IV generan contribuciones negativas."
#| echo: false
#| message: false
#| warning: false
#| fig-width: 10
...
generar_panel <- function(tipo = c("positiva", "nula", "negativa"), n = 70) {
  tipo <- match.arg(tipo)
  x <- rnorm(n)
  if (tipo == "positiva") {
    y <- 0.85 * x + rnorm(n, sd = 0.45)
...
paneles <- list(
  "Covarianza positiva" = generar_panel("positiva"),
  "Covarianza cercana a cero" = generar_panel("nula"),
  "Covarianza negativa" = generar_panel("negativa")
)

par(mfrow = c(1, 3), mar = c(4, 4, 3, 1))

for (titulo in names(paneles)) {
  d <- paneles[[titulo]]
  plot(
    d$x, d$y,
    pch = 19, cex = 0.7,
    xlab = expression(X - bar(X)),
    ylab = expression(Y - bar(Y)),
    main = titulo
  )
  abline(h = 0, v = 0, lty = 2)
  usr <- par("usr")
  text(usr[2] * 0.78, usr[4] * 0.82, "I", font = 2)
  text(usr[1] * 0.78, usr[4] * 0.82, "II", font = 2)
  text(usr[1] * 0.78, usr[3] * 0.82, "III", font = 2)
  text(usr[2] * 0.78, usr[3] * 0.82, "IV", font = 2)
}

par(mfrow = c(1, 1))
```

La figura permite observar que el **signo** de la covarianza está
relacionado con la ubicación de los puntos respecto a las medias. Sin
embargo, su **magnitud depende de las unidades y de la escala** de las
variables. Por esta razón, no es correcto interpretar una covarianza
numéricamente grande o pequeña utilizando los límites $-1$ y $1$; esos
límites corresponden al coeficiente de correlación.

![Esquema conceptual de la covarianza y de cómo dos variables pueden
variar conjuntamente.](img/covarianza.png){#fig-covarianza}
...
**Interpretación:** La covarianza calculada
($\operatorname{Cov}(X,Y)=0.062$) entre la longitud de la pierna y la
longitud del pie tiene signo positivo, lo que indica que, en promedio,
las desviaciones de ambas variables respecto a sus medias tienden a
aportar en el mismo sentido. No obstante, **la magnitud de una
covarianza no debe interpretarse directamente como fuerte o débil**,
porque depende de las unidades y escalas de las variables analizadas.

Para valorar la fuerza de la asociación en una escala estandarizada se
utiliza el **coeficiente de correlación de Pearson**. En este caso, el
valor previamente obtenido ($r = 0.012$) se encuentra muy próximo a cero
y evidencia una asociación lineal prácticamente nula entre las
variables. La comparación permite observar la relación conceptual entre
ambas medidas: la covarianza aporta la variación conjunta en el
numerador y la correlación la estandariza mediante las desviaciones
estándar.
