...
central, la dispersión, la posición y la forma de las distribuciones. Sin
embargo, en los problemas reales las variables rara vez se comportan de
manera aislada. Con frecuencia, el interés estadístico consiste en
...
diferencias entre grupos o si algunas características pueden contribuir a
explicar el comportamiento de otra variable. Este tipo de preguntas
requiere pasar de la descripción individual de los datos al estudio de las
**relaciones y asociaciones entre variables**.

El **análisis de asociación** reúne herramientas estadísticas orientadas a
explorar, describir, cuantificar e interpretar estas relaciones. Cuando el
estudio se concentra específicamente en dos variables se habla de
**análisis bivariado**. Dependiendo de la naturaleza de las variables y del
propósito del análisis, pueden estudiarse relaciones entre variables
cuantitativas, asociaciones entre variables categóricas, diferencias entre
grupos o relaciones de tipo explicativo y predictivo.

Es importante distinguir que **asociación no significa causalidad**.
Identificar que dos variables presentan un patrón conjunto no implica que
una sea necesariamente la causa de la otra. La interpretación debe
...
Spearman, las tablas de contingencia y los modelos de regresión lineal**.
Estas técnicas permiten estudiar la dirección, intensidad y estructura de
las relaciones observadas, así como construir modelos básicos para
describir o explicar el comportamiento conjunto de las variables.
...
progresivamente la representación gráfica, las medidas de asociación y los
...
técnica estadística apropiada, su implementación en R y la interpretación
de los resultados.

Este análisis constituye además una transición hacia métodos estadísticos
de mayor complejidad. La comprensión de las relaciones entre variables
proporciona fundamentos para estudiar posteriormente modelos de regresión
múltiple, técnicas multivariadas y procedimientos utilizados en analítica
de datos y modelamiento estadístico.
...
**explorar visualmente → cuantificar la variación conjunta → estandarizar
la asociación → modelar la relación → estudiar asociaciones entre
categorías**.

Esta secuencia permite comprender no solo cómo calcular cada estadístico,
sino también qué pregunta responde, bajo qué condiciones puede utilizarse
y cómo interpretar sus resultados dentro del contexto del problema.
...
facilita la comparación de resultados y permite comprender cómo distintas
herramientas estadísticas pueden estudiar una realidad desde perspectivas
complementarias.

La base contiene información antropométrica correspondiente a estudiantes
universitarios e incluye variables cuantitativas y categóricas. Entre las
variables utilizadas en los ejemplos del capítulo se encuentran la
**altura**, el **peso corporal**, el **índice de masa corporal (IMC)**, la
**longitud de la pierna**, la **longitud del pie**, la **longitud de la
mano** y el **género**, además de variables derivadas de clasificación
antropométrica presentes en el conjunto de datos.
...
(@fig-dosvariables). Por esta razón,
...
![Esquema para la selección de técnicas de análisis de asociación según la naturaleza de las variables.](img/dosvariables.png){#fig-dosvariables}
