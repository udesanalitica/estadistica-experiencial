...
Describen cualidades, atributos o categorías que no pueden medirse numéricamente, pero sí clasificarse.

-   **Nominales:** Sus categorías no admiten un orden natural (ej: sexo, estado civil).

-   **Ordinales:** Sus categorías indican un orden o jerarquía (ej: nivel socioeconómico, grado académico).

### Variables cuantitativas

Son aquellas que toman valores numéricos y se originan mediante procesos de conteo o medición .

-   **Discretas:** Asociadas a procesos de conteo, asumen valores enteros aislados (ej: número de hijos, número de artículos defectuosos).

-   **Continuas:** Asociadas a procesos de medición, pueden tomar cualquier valor dentro de un intervalo determinado (ej: peso, estatura, edad),.

## Escalas de medición

La naturaleza de las observaciones determina el nivel de medición, lo cual es fundamental para elegir el método estadístico apropiado (Tabla 1)[@armitage2002statistical; @kleinbaum1982epidemiologic].

::: {#tbl-variables tbl-cap="Clasificación de variables"}
| Escala | Característica | Ejemplo |
|------------------------|------------------------|------------------------|
| **Nominal** | Clasificación en categorías mutuamente excluyentes sin orden. | Género, lugar de nacimiento. |
| **Ordinal** | Existe un orden o jerarquía entre niveles, pero la distancia entre ellos no es constante. | Estrato socioeconómico, grado militar. |
| **Intervalo** | Orden con distancias iguales entre valores. El cero es arbitrario y no indica ausencia de la propiedad. | Temperatura (Celsius o Fahrenheit). |
| **Razón** | Posee todas las propiedades anteriores y un **cero absoluto** que indica ausencia total de la característica. | Edad, peso, ingresos, longitud. |
:::

## Importancia de la identificación de variables
...
A coantinutación se analiza un conjunto de datos de estudiantes de primer ingreso en la universidad, donde se desea identificar características como edad, sexo, estrato socioeconómico y horas de estudio.
